...
// 文档上传接口
router.post('/documents/upload', auth, checkRole('admin'), upload.single('document'), async (req, res, next) => {
  try {
    ...
    console.log('提取的数据:', { title, description, category, secondaryTags, type, status })
    
    // 处理二级标签（可能是JSON字符串）
    let parsedSecondaryTags = []
    if (secondaryTags) {
      try {
        parsedSecondaryTags = Array.isArray(secondaryTags) 
          ? secondaryTags 
          : JSON.parse(secondaryTags)
      } catch (e) {
        // 如果解析失败，按逗号分割
        parsedSecondaryTags = secondaryTags.split(',').map(tag => tag.trim()).filter(Boolean)
      }
    }
    
    // 确保标题不为空，如果为空则使用默认值
    const documentTitle = title && title.trim() ? title.trim() : '未命名文档'
    ...
    if (req.file) {
      ...
      documentType = path.extname(req.file.originalname).substring(1).toUpperCase()
      ...
      console.log('文件处理结果:', { filePath, fileSize, downloadUrl, documentType })
    } else {
      // 没有文件上传，使用占位符
      filePath = `/uploads/documents/${documentTitle}.${documentType.toLowerCase()}`
      fileSize = 1024000 // 1MB 占位符
      downloadUrl = filePath
      if (documentType === 'PDF') {
        previewUrl = `/uploads/documents/preview/${documentTitle}.pdf`
      }
      
      console.log('使用占位符文件信息:', { filePath, fileSize, downloadUrl, documentType })
    }
    
    const documentData = {
      title: documentTitle,
      description: description || '',
      category: category || '模板资源',
    ...
    console.log('创建文档数据:', documentData)
